...
##junction annotation##
hg38junc<-read.delim("hg38_junction_HAVANA.ENSEMBL.txt",header = TRUE,stringsAsFactors = FALSE)
##subset by lncRNA##
...
su2c<-read.delim("05115024-T-A-1_SI_4737.janno",header=TRUE,stringsAsFactors = FALSE)
...
dancr_juncNor<-data.frame(sample="05115024-T-A-1_SI_4737.janno",gene="DANCR",score=dancr_score,total=total,scoreN=round(dancr_score/total*10^6,0))
write.table(dancr_juncNor,paste0("05115024-T-A-1_SI_4737.janno",".dancr.txt"),col.names = FALSE,row.names = FALSE,quote = FALSE,sep="\t")
...
malat1_juncNor<-data.frame(sample="05115024-T-A-1_SI_4737.janno",gene="MALAT1",score=malat1_score,total=total,scoreN=round(malat1_score/total*10^6,0))
write.table(malat1_juncNor,paste0("05115024-T-A-1_SI_4737.janno",".malat1.txt"),col.names = FALSE,row.names = FALSE,quote = FALSE,sep="\t")
