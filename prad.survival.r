...
df.rpkm<-read.delim(input1,header = TRUE,stringsAsFactors = FALSE)
df.surv<-read.delim(input2,header = TRUE,stringsAsFactors = FALSE)
...
fit1<-survfit(Surv(DSS_time,DSS)~DANCR,data=prad.surv.status)
#summary(fit1)
...
ggsave(paste0(lncrnas_names[i-12],".DS.pdf"), plot = print(res), width = 8, height = 8, dpi = 500)



##overall survival##
fit1<-survfit(Surv(OS_time,OS)~TUG1,data=prad.surv.status)
summary(fit1)
ggsurvplot(fit1,data=prad.surv.status,
           conf.int=TRUE,
           pval = TRUE,
           fun="pct",
           risk.table = TRUE,
...
##disease free##
fit1<-survfit(Surv(DFI_time,DFI)~TUG1,data=prad.surv.status)
summary(fit1)
ggsurvplot(fit1,data=prad.surv.status,
           conf.int=TRUE,
           pval = TRUE,
...
##progress free##
fit1<-survfit(Surv(PFI_time,PFI)~TUG1,data=prad.surv.status)
summary(fit1)
ggsurvplot(fit1,data=prad.surv.status,
           conf.int=TRUE,
           pval = TRUE,
